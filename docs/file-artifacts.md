...
| `file_artifact_non_identity_attrs` | IMPHASH / TLSH / SSDEEP (not used as auto-merge evidence) |
...
No additive list index migration (`132_*`) is shipped yet. Fixture-scale `EXPLAIN (ANALYZE, BUFFERS)` on the disposable harness did not justify a new index; re-measure at 1–5k identities under expected load.
...
Global empty-artifact deletes are not part of attach/backfill; hash uniqueness remains source of truth.
...
Auto-merge is not driven by: file name, size, MIME, family, tags, IMPHASH, TLSH, SSDEEP, or timestamps alone.
...
- Direct observations are limited to hashes the source actually sent
...
1. Deploy migration `131` (additive; compatible with flags off)
...
4. Run operational SQL checks (below)
...
2. Set read off as well if tables will go stale
3. Leave tables in place; no drop migration in this phase
...
Hard fail unless `ALLOW_FILE_ARTIFACT_DB_TESTS=1`, `DB_NAME` contains `_test`, host is `127.0.0.1`/`localhost`, and `NODE_ENV` ≠ `production`.
...
Add `132_*` indexes only with a real plan justification. Shipped `131` is immutable if any environment may have applied it.
