...
|--------|---------------|-----------------|----------------|
| Beta prerelease | `v0.1.1-beta.11` | `0.1.1-beta.11` | Prerelease |
...
### Development / source-build workflow
...
Official release deployments pull pre-built images; `--build` is not part of this path.
...
## Update checks and in-place upgrades

Public-beta update checks and the safe CLI upgrade workflow are documented in [`docs/upgrade.md`](upgrade.md).

- Channel pointer: `updates/<channel>.json` (HTTPS JSON; default URL is overridable via `UPDATE_MANIFEST_URL`)
- Admin UI: **System → Settings → Updates** (check only — no browser upgrade)
- Host upgrade: `sudo ./installation.sh --upgrade` / `--upgrade-to <semver>` / `--dry-run`
...
Use the safe CLI upgrade workflow in [`docs/upgrade.md`](upgrade.md):
...
Upgrades pin exact GHCR images from `release-manifest.json`, run migrations explicitly, and preserve `.env` / volumes.
