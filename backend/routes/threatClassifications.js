...
import { loadMitreMappingsByClassificationId } from '../lib/threatClassifications/seed.js';
...
function serializeMitreAttackMapping(row) {
  return {
    id: row.attack_id,
...
function serializeThreatClassification(row, mitreMappings = []) {
  if (!row) return null;
  const payload = {
    id: row.id,
    name: row.name,
  ...
  if (mitreMappings.length) {
    payload.mitre_attack = mitreMappings.map(serializeMitreAttackMapping);
  }
  return payload;
}
...
export function registerThreatClassificationRoutes(app, pool, audit) {
  app.get('/api/threat-classifications', async (_req, res) => {
    try {
      ...
      const mitreByClassId = await loadMitreMappingsByClassificationId(pool);
      const { rows: classRows } = await pool.query(
        `SELECT id, slug FROM threat_classifications WHERE active = TRUE`
      );
      const slugToId = new Map(classRows.map((r) => [r.slug, String(r.id)]));
      const enriched = list.map((item) => {
        const classId = slugToId.get(item.value);
        const mappings = classId ? (mitreByClassId.get(classId) || []) : [];
        if (!mappings.length) return item;
        return {
          ...item,
          mitre_attack: mappings.map(serializeMitreAttackMapping)
        };
      });
  ...
  app.get('/api/admin/threat-classifications', requireRole(ROLES.ADMIN), async (req, res) => {
    ...
    try {
      ...
      const mitreByClassId = await loadMitreMappingsByClassificationId(pool);
      return res.json({
        threat_classifications: rows.map((row) => serializeThreatClassification(
          row,
          mitreByClassId.get(String(row.id)) || []
        ))
      });
    } catch (err) {
      return res.status(500).json({ success: false, error: err.message });
  ...
  app.post('/api/admin/threat-classifications/reorder', requireRole(ROLES.ADMIN), async (req, res) => {
    ...
    try {
      ...
      return res.json({
        success: true,
        threat_classifications: afterRows.map((row) => serializeThreatClassification(
          row,
          mitreByClassId.get(String(row.id)) || []
        ))
      });
