...
const priorChunkedFlag = process.env.PUBLISHED_FEED_CHUNKED_ENABLED;
test.before(() => {
  process.env.PUBLISHED_FEED_CHUNKED_ENABLED = 'false';
});
test.after(() => {
  if (priorChunkedFlag === undefined) delete process.env.PUBLISHED_FEED_CHUNKED_ENABLED;
  else process.env.PUBLISHED_FEED_CHUNKED_ENABLED = priorChunkedFlag;
});
