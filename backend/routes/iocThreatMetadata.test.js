...
describe('buildThreatMetadataFields — details/list parity', () => {
  it('includes legacy column when junction empty + feed tag classifications (URLHaus dropper_downloader case)', async () => {
    ...
    assert.deepEqual(
      ...
      'details must surface legacy malware + feed dropper_downloader like the list'
    );
    assert.ok(fields.threat_classifications.some((x) => x.value === 'malware'));
    assert.ok(fields.threat_classifications.some((x) => x.value === 'dropper_downloader'));
  });
...
describe('batchLoadFeedClassifications', () => {
  ...
  it('returns feed classifications from evidence note tags', async () => {
    ...
    assert.ok(slugs.includes('dropper_downloader'), 'malware_download tag should map to dropper_downloader classification');
  });
  ...
  it('deduplicates across multiple evidence rows', async () => {
    ...
    const slugs = classes.map((c) => c.value);
    assert.equal(slugs.filter((s) => s === 'dropper_downloader').length, 1, 'should deduplicate dropper_downloader');
  });
});
