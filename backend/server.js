...
const FEED_JOB_TYPE_BY_KEY = {
  ...
  'certpl-warning-list': 'certpl_import'
};
...
app.get('/api/auth/me', async (req, res) => {
  ...
  if (req.user?.id != null) {
    try {
      const { rows } = await pool.query(
        'SELECT public_id, username, role, must_change_password, is_system_admin FROM users WHERE id = $1',
        [Number(req.user.id)]
      );
      if (rows.length) {
        ...
        isSystemAdmin = rows[0].is_system_admin === true;
      }
    } catch {
      // fall through — isSystemAdmin stays false (fail closed)
    }
  }
...
app.post('/api/ioc/stats/refresh', requireRole(ROLES.ADMIN, ROLES.ANALYST), async (req, res) => {
  try {
    const result = await queueIocListStatsRefresh(pool, { force: false });
    if (!result.queued) {
      return res.status(409).json({
        error: IOC_STATS_RECALCULATION_IN_PROGRESS,
        code: IOC_STATS_RECALCULATION_IN_PROGRESS,
        message: 'IOC statistics recalculation is already in progress.',
        in_progress: true
      });
    }
    return res.status(202).json({
      ...
      queued: true,
      in_progress: true,
      message: 'IOC statistics recalculation started. Updated stats will appear shortly.'
    });
  } catch (err) {
...
app.listen(port, async () => {
  ...
  import('./lib/publishedFeedChunkGeneration.js')
    .then(async ({
      ...
      const { reconcileSingleWindowPublicGenerations } = await import('./lib/feedPublisherService.js');
      await reconcileSingleWindowPublicGenerations(pool);
      await reconcileRetiredLegacySnapshotsForActiveChunkGenerations(pool);
      await reconcileOrphanGenerationChunkLinks(pool);
      await purgeUnreachablePublishedFeedChunks(pool);
  ...
  try {
    updateCheckService.startBackgroundChecks();
  } catch {
    /* ignore */
  }
});
