...
import {
  ...
  reconcileBundledThreatClassifications
} from './lib/threatClassifications/seed.js';
...
async function runMigrations() {
  ...
  try {
    ...
    printBundledReconcileSummary(seedResult, { dryRun: false });
    if (seedResult.applied) {
      console.log(`[migrate] threat actors: inserted=${seedResult.applied.inserted}, updated=${seedResult.applied.updated}`);
    }

    console.log('[migrate] reconciling bundled threat classification taxonomy');
    const tcSeedResult = await reconcileBundledThreatClassifications(client, { dryRun: false });
    printBundledThreatClassificationSummary(tcSeedResult, { dryRun: false });
  } catch (err) {
