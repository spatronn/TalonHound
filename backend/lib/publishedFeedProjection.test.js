...
describe('publishedFeedProjection helpers', () => {
  ...
  it('canUseIncrementalRefresh is conservative for max_items and windows', () => {
    ...
    assert.equal(canUseIncrementalRefresh({ ...ready, time_window: '7d' }, { snapshotWindow: '7d' }), true);
...
describe('decideRefreshMode', () => {
  ...
  it('uses incremental for sliding windows when projection ready', () => {
    assert.equal(decideRefreshMode(
      { projection_status: 'ready', time_window: '7d', max_items: null },
      { streamingEnabled: true, incrementalEnabled: true, snapshotWindow: '7d' }
    ), 'incremental');
  });
  ...
  it('splits oversized batches below the Postgres bind protocol limit', async () => {
    ...
    const db = {
      async query(sql, params) {
        ...
        return { rowCount: params.length / 16 };
      }
    };
    ...
    assert.equal(calls[0], PROJECTION_UPSERT_MAX_ROWS * 16);
    assert.equal(calls[1], (5000 - PROJECTION_UPSERT_MAX_ROWS) * 16);
  });
});
