...
test('urlhaus: maps botnet_cc to command_and_control', () => {
  assert.equal(resolveClassificationFromFeed('urlhaus-abusech', 'botnet_cc'), 'command_and_control');
});

test('urlhaus: maps malware_download to dropper_downloader', () => {
  assert.equal(resolveClassificationFromFeed('urlhaus-abusech', 'malware_download'), 'dropper_downloader');
});
...
test('threatfox: maps botnet_cc to command_and_control', () => {
  assert.equal(resolveClassificationFromFeed('threatfox-abusech', 'botnet_cc'), 'command_and_control');
});

test('threatfox: maps payload_delivery to dropper_downloader', () => {
  assert.equal(resolveClassificationFromFeed('threatfox-abusech', 'payload_delivery'), 'dropper_downloader');
});
...
test('all feeds: returns null when rawValue is null', () => {
  for (const feedKey of ['urlhaus-abusech', 'threatfox-abusech', 'malwarebazaar-abusech', 'phishtank-opendnsrr', 'alienvault-otx', 'et-blockrules', 'certpl-warning-list']) {
    assert.equal(resolveClassificationFromFeed(feedKey, null), null, `${feedKey} should return null for null rawValue`);
  }
});

test('certpl-warning-list: returns null (no feed-specific classification map)', () => {
  assert.equal(resolveClassificationFromFeed('certpl-warning-list', 'phishing'), null);
  assert.equal(resolveClassificationFromFeed('certpl-warning-list', 'malware'), null);
});
