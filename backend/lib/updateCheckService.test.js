...
test('N2. Docker placeholder TALONHOUND_VERSION=dev compares using VERSION file SemVer', async () => {
  await withEnv({
    ...
    const service = createUpdateCheckService({
      fetchImpl: async () => jsonResponse(betaManifest({
        latest: '0.1.1-beta.10',
        release_url: 'https://github.com/spatronn/TalonHound/releases/tag/v0.1.1-beta.10'
      }))
    });
    const status = await service.check({ force: true });
    assert.equal(status.currentVersion, '0.1.1-beta.10');
    assert.equal(status.latestVersion, '0.1.1-beta.10');
    assert.equal(status.status, 'up_to_date');
  });
});
...
test('N4. beta.2 install detects beta.3 update on beta channel', async () => {
  await withEnv({
    ...
    const service = createUpdateCheckService({
      fetchImpl: async () => jsonResponse(betaManifest({
        latest: '0.1.1-beta.4',
        release_url: 'https://github.com/spatronn/TalonHound/releases/tag/v0.1.1-beta.4'
      }))
    });
    const status = await service.check({ force: true });
    assert.equal(status.currentVersion, '0.1.0-beta.2');
    assert.equal(status.latestVersion, '0.1.1-beta.4');
    assert.equal(status.status, 'update_available');
    assert.equal(status.releaseUrl, 'https://github.com/spatronn/TalonHound/releases/tag/v0.1.1-beta.4');
  });
});

test('N3. beta.1 install detects beta.2 update on beta channel', async () => {
  await withEnv({
    TALONHOUND_VERSION: '0.1.0-beta.1',
    UPDATE_CHANNEL: 'beta',
    UPDATE_MANIFEST_URL: 'https://example.com/updates/beta.json'
    ...
    assert.equal(status.latestVersion, '0.1.0-beta.2');
    assert.equal(status.status, 'update_available');
    assert.equal(status.releaseUrl, 'https://github.com/spatronn/TalonHound/releases/tag/v0.1.0-beta.2');
  });
});
