...
export async function cleanupSupersededArtifacts(cfg, feedId, currentStoragePaths) {
  ...
  for (const f of files) {
    ...
    // In-flight multi-format streaming temps (*.json.body / *.stix.body). Never delete
    // these here: a concurrent retention sweep can unlink a still-open zero-byte body
    // while generateFeedArtifact holds the write FD; later pipeFileInto(path) then
    // fails with ENOENT (observed on large hash force rebuilds).
    if (f.endsWith('.body')) continue;
    const st = await statArtifact(p);
    // Abandoned zero-byte finals (failed/crash mid-promote) are never valid artifacts.
    if (st && st.size === 0) {
      await removeFileQuiet(p);
      removed += 1;
      continue;
