...
import {
  ...
  buildProjectionScanSql as buildWindowProjectionScanSql,
  isSlidingWindow,
...
/** Postgres bind protocol uses a uint16 parameter count. 16 placeholders per row. */
export const PROJECTION_UPSERT_PARAMS_PER_ROW = 16;
...
export function canUseIncrementalRefresh(feed, { force = false, filtersChanged = false, snapshotWindow = null } = {}) {
  ...
  if (window && window !== 'all') {
    if (!isSlidingWindowIncrementalEnabled() || !isSlidingWindow(window)) return false;
  }
  return true;
}

export { BASE_PROJECTION_WINDOW };
...
async function upsertProjectionBatchChunk(db, rows) {
  ...
  for (const r of rows) {
    values.push(
      `($${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++},$${i++}::jsonb,$${i++},$${i++},$${i++},published_feed_reconciliation_bucket($${i++}),NOW())`
    );
    params.push(
      r.feed_id,
  ...
  const sql = `
    INSERT INTO published_feed_items (
      ...
      content_fingerprint, partition_identity, chunk_key, reconciliation_bucket, updated_at
    ) VALUES ${values.join(',')}
    ON CONFLICT (feed_id, snapshot_window, identity_key) DO UPDATE SET
      ...
      reconciliation_bucket = EXCLUDED.reconciliation_bucket,
      updated_at = NOW()
    WHERE published_feed_items.content_fingerprint IS DISTINCT FROM EXCLUDED.content_fingerprint
       ...
       OR published_feed_items.reconciliation_bucket IS DISTINCT FROM EXCLUDED.reconciliation_bucket`;
...
export async function setFeedProjectionState(db, feedId, patch) {
  ...
  if (patch.reconciliation_slice !== undefined) add('reconciliation_slice', patch.reconciliation_slice);
  if (patch.reconciliation_cursor !== undefined) add('reconciliation_cursor', patch.reconciliation_cursor);
...
export function buildProjectionScanSql(feedId, artifactWindow, asOf = null) {
  if (isSlidingWindowIncrementalEnabled()) {
    return buildWindowProjectionScanSql(feedId, artifactWindow, asOf);
  }
  return {
    sql: `
    ...
    params: [feedId, artifactWindow]
  };
}

export async function countProjectionItems(db, feedId, window, asOf = null) {
  if (isSlidingWindowIncrementalEnabled() && String(window).toLowerCase() !== BASE_PROJECTION_WINDOW) {
    const { countProjectionItemsForWindow } = await import('./publishedFeedWindowEligibility.js');
    return countProjectionItemsForWindow(db, feedId, window, asOf);
  }
