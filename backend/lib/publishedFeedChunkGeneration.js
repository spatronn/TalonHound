...
import { resolvePublishedFeedFormats, resolveJsonIncludeFlags } from './feedPublisherService.js';
...
async function generateChunkFiles(db, feed, window, chunkCount, affectedChunkKeys, formats, cfg) {
...
async function writeRecencyHead(db, cfg, feed, window, id) {
  const { rows } = await db.query(
    `SELECT txt_value
     FROM published_feed_items
     WHERE feed_id = $1 AND snapshot_window = $2
     ...
     LIMIT $3`,
    [feed.id, window, RECENCY_HEAD_ITEMS]
  );
...
export async function buildAndActivateChunkGeneration(db, feed, {
  ...
  const full = !parent || !affectedChunkKeys;
  const affected = full
  ...
  await db.query(
    ...
    [
      ...
      JSON.stringify(formats), configHash, fullRebuildReason, JSON.stringify(metrics)
    ]
  );
  throwIfInjected(failAt, 'after_generation_insert');

  const t0 = Date.now();
  const generated = await generateChunks(db, feed, window, chunkCount, affected, formats, cfg);
  ...
  const recencyHead = formats.includes('txt')
    ? await writeRecencyHead(db, cfg, feed, window, id)
    : null;
  for (const format of formats) {
    // eslint-disable-next-line no-await-in-loop
