...
describe('retireLegacyMonolithicSnapshotsForWindow', () => {
  it('clears storage_path pointers for the window and marks chunk_owned', async () => {
    const seen = [];
    const db = {
      async query(sql, params = []) {
        seen.push(String(sql));
        if (String(sql).includes('UPDATE published_feed_snapshots')) {
          assert.deepEqual(params, [11, 'all']);
          assert.match(String(sql), /chunk_owned/);
          return {
            rows: [
              { storage_path: '11/old.json' },
              { storage_path: '11/old.txt' }
            ]
          };
        }
    ...
    const result = await retireLegacyMonolithicSnapshotsForWindow(db, 11, 'all');
    assert.equal(result.retired, 2);
    assert.deepEqual(result.storagePaths, ['11/old.json', '11/old.txt']);
    assert.ok(seen.some((s) => s.includes('storage_path = NULL')));
    assert.ok(seen.some((s) => s.includes('chunk_owned')));
  });
});
