...
async function backfill(client) {
  ...
  try {
        ...
        await client.query(
          `UPDATE published_feed_items p
           SET partition_identity = x.partition_identity,
               ...
               reconciliation_bucket = published_feed_reconciliation_bucket(x.partition_identity)
    ...
    const verify = await client.query(
      `SELECT COUNT(*) FILTER (WHERE partition_identity IS NULL OR chunk_key IS NULL)::bigint AS missing_chunks,
              COUNT(*) FILTER (WHERE partition_identity IS NOT NULL AND reconciliation_bucket IS NULL)::bigint AS missing_buckets,
              COUNT(*)::bigint AS total
       FROM published_feed_items
       WHERE feed_id = $1 AND snapshot_window = $2`,
      [feedId, windowName]
    );
    if (Number(verify.rows[0]?.missing_chunks || 0) !== 0) {
      throw new Error(`Chunk backfill incomplete: ${verify.rows[0].missing_chunks} rows missing chunk metadata`);
    }
    if (Number(verify.rows[0]?.missing_buckets || 0) !== 0) {
      throw new Error(`Reconciliation bucket backfill incomplete: ${verify.rows[0].missing_buckets} rows missing`);
    }
    await client.query(
