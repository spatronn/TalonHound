...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a.startsWith('--trigger-type=')) out.flags.triggerType = a.slice('--trigger-type='.length);
    else if (a === '--trigger-type') out.flags.triggerType = argv[++i];
...
/** Safe backup id for CLI overrides (upgrade naming). */
function validateBackupId(value) {
  const id = String(value || '').trim();
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]{2,120}$/.test(id)) {
    return { ok: false, error: 'Invalid --backup-id (allowed: letters, digits, ._- ; 3-121 chars)' };
  }
  return { ok: true, id };
}

function validateTriggerType(value) {
  const t = String(value || 'manual').trim();
  if (!/^[a-z][a-z0-9_]{1,32}$/.test(t)) {
    return { ok: false, error: 'Invalid --trigger-type' };
  }
  return { ok: true, triggerType: t };
}
...
async function cmdCreate(args) {
  ...
  return withPool(async (pool) => {
    ...
    if (args.flags.backupId) {
      const checked = validateBackupId(args.flags.backupId);
      if (!checked.ok) {
    ...
    const trigger = validateTriggerType(args.flags.triggerType || 'manual');
    if (!trigger.ok) {
