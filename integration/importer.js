      ...
      for (const e of duplicateEntries) {
        ...
        if (existing.status === 'unchanged') {
          ...
          await storeFeedSourceEvidence(client, {
            ...
            category: e.category ?? null,
            note: e.note ?? null,
            confidence: e.confFields?.confidence ?? null
          });
        } else {
          out.duplicate += 1;
...
function buildCertPlBatchEntry(entry, sourceName, feedDefaultConfidence) {
  return {
    observable: entry.observable,
    observableType: 'domain',
    sourceName,
    sourceUrl: CERTPL_DOMAINS_URL,
    feedDefaultConfidence,
 ...
 *   omits a previously seen domain (TalonHound expiry remains authoritative)
 * - Uses content-hash + add-diff like PhishTank for subsequent-run efficiency
 */
export async function runCertPlImport(options = {}) {
  const { signal } = options;
  const triggeredBy = resolveTriggeredBy(options);
  const client = await pool.connect();
  const startedAt = new Date();
  ...
  const suppressionStats = createSuppressionStats();
  const metrics = createImportMetrics();
  const txMeta = { source_key: CERTPL_FEED_KEY, signal };
  const sourceName = config.certPlSourceName || CERTPL_SOURCE_NAME;

  try {
    throwIfAborted(signal);
    const lockResult = await client.query('SELECT pg_try_advisory_lock($1) AS acquired', [CERTPL_ADVISORY_LOCK]);
    if (!lockResult.rows[0]?.acquired) {
      return { skipped: true, reason: 'lock_not_acquired' };
    }
    ...
    return await withImportOptimizationContext(client, async () => {
      const runInsert = await client.query(
        `INSERT INTO integration_runs (job_type, status, started_at, triggered_by)
         VALUES ('certpl_import', 'running', clock_timestamp(), $1)
         RETURNING id`,
        [triggeredBy]
      );
      runId = runInsert.rows[0].id;
      ...
      const fetched = await fetchCertPlDomains({
        url: CERTPL_DOMAINS_URL,
        timeoutMs: config.certPlTimeoutMs,
        maxRetries: config.certPlMaxRetries,
        signal
      });
      ...
      if (previousHash && previousHash === currentHash) {
        const summary = {
          ...
          upstream_deleted_skipped: parseStats.upstream_deleted_skipped,
          invalid_skipped: parseStats.invalid_skipped,
          duplicate_normalized_collapsed: parseStats.duplicate_normalized_collapsed,
          unique_domains: entries.length,
          added: 0,
        ...
        console.log(
          `[integration-import] job=certpl_import runId=${runId} feed=certpl fetched=${parseStats.fetched} active=${parseStats.active} upstream_deleted_skipped=${parseStats.upstream_deleted_skipped} invalid_skipped=${parseStats.invalid_skipped} noop=same_hash duration_ms=${Date.now() - startedAt.getTime()}`
        );
      ...
      const batchEntries = addedEntries.map((e) => buildCertPlBatchEntry(e, sourceName, feedDefaultConfidence));

      for (let i = 0; i < batchEntries.length; i += batchSize) {
        throwIfAborted(signal);
        const batch = batchEntries.slice(i, i + batchSize);
        await withPgTransaction(client, 'certpl_import_batch', async (tx) => {
          mergeBatchInsertMetrics(
            metrics,
            await batchInsertIocs(tx, batch, 'domain', suppressionStats, signal, { duplicateHandling: 'count_only' })
          );
        }, { ...txMeta, batch: Math.floor(i / batchSize) + 1 });
      }
      ...
      await finalizeIntegrationRun(client, runId, metrics, 'success', { summary });
      logImportSuppressionSummary('certpl_import', runId, suppressionStats, metrics.toJSON());
      console.log(
        `[integration-import] job=certpl_import runId=${runId} feed=certpl fetched=${parseStats.fetched} active=${parseStats.active} upstream_deleted_skipped=${parseStats.upstream_deleted_skipped} invalid_skipped=${parseStats.invalid_skipped} unique=${entries.length} added=${addedEntries.length} inserted=${metrics.records_inserted} updated=${metrics.records_updated} duplicate=${metrics.records_duplicate} unchanged=${metrics.records_unchanged} duration_ms=${Date.now() - startedAt.getTime()}`
      );
      return withSuppressionStats({ ok: true, runId, summary }, suppressionStats, metrics);
    });
  } catch (err) {
    if (runId) {
      await failIntegrationRun(client, runId, err?.message || String(err), metrics);
    }
    throw err;
  } finally {
    try {
      await client.query('SELECT pg_advisory_unlock($1)', [CERTPL_ADVISORY_LOCK]);
    } catch {
      // ignore
    }
    client.release();
