...
import { runHourlyImport, runUsomImport, runUrlhausImport, runThreatfoxImport, runMalwareBazaarImport, runPhishtankImport, runAlienvaultOtxImport, runCertPlImport } from './importer.js';
...
function resolveIntegrationKey(job) {
  ...
  if (job?.name === 'certpl-import') return 'certpl-warning-list';
  return 'unknown';
}
...
async function runImportForJob(job, { signal, triggeredBy } = {}) {
  ...
  if (job.name === 'certpl-import') return runCertPlImport(opts);
