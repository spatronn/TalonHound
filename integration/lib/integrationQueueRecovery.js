...
export async function recoverStaleRunningJobs(pool, { logPrefix = '[integration-worker]', queue = null, dryRun = false } = {}) {
  ...
  for (const row of runningQ.rows) {
    ...
    console.log(
      `${logPrefix} Marked stale running job as failed job_id=${row.job_id} source=${row.integration_key} failure_type=${classification.failureType} timeout_ms=${classification.timeoutMs || '-'} timeout_source=${classification.timeoutSource || '-'} last_heartbeat_at=${row.heartbeat_at || '-'} running_age_ms=${classification.ageMs} recovery_reason=${classification.failureType}`
    );
  }
