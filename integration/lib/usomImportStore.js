...
/** One canonical IOC row per staged observable — built once per finalize transaction. */
const CANONICAL_STAGE_TABLE = 'usom_stage_canonical';
...
/**
 * Resolve one canonical IOC row per staged observable once per finalize transaction.
 * Replaces repeated per-row LATERAL lookups across classify/upsert/evidence/policy steps.
 */
async function buildStageCanonical(client) {
  await client.query(`DROP TABLE IF EXISTS ${CANONICAL_STAGE_TABLE}`);
  await client.query(
    `CREATE TEMP TABLE ${CANONICAL_STAGE_TABLE} ON COMMIT PRESERVE ROWS AS
     SELECT DISTINCT ON (s.observable_type, s.observable)
            ...
            s.note,
            s.provider_metadata,
            s.provider_fingerprint,
            s.provider_record_timestamp,
  ...
  await client.query(
    `CREATE UNIQUE INDEX ON ${CANONICAL_STAGE_TABLE} (observable_type, observable)`
  );
  await client.query(
    `CREATE INDEX ON ${CANONICAL_STAGE_TABLE} (ioc_item_id, observable_type)
     WHERE ioc_item_id IS NOT NULL`
  );
}
...
async function insertNewIocs(client, feed) {
  ...
  await client.query(
    `INSERT INTO ioc_observables (ioc_public_id, observable_type, observable_value)
     SELECT DISTINCT ON (c.public_id, c.observable_type, c.observable)
            c.public_id,
            c.observable_type,
            CASE
              WHEN c.observable_type IN ('domain', 'url') THEN lower(c.observable)
              ELSE c.observable
            END
     FROM ${CANONICAL_STAGE_TABLE} c
...
async function adoptMissingFingerprints(client, feedId) {
  const { rowCount } = await client.query(
    `UPDATE ioc_feed_memberships m
     SET content_fingerprint = c.provider_fingerprint
     FROM ${CANONICAL_STAGE_TABLE} c
     WHERE m.ioc_item_id = c.ioc_item_id
       AND m.ioc_observable_type = c.observable_type
       AND m.feed_id = $1::uuid
...
async function upsertMemberships(client, feedId, seenAt) {
  const { rowCount } = await client.query(
    `INSERT INTO ioc_feed_memberships (
     ...
     SELECT c.ioc_item_id, c.observable_type, $1::uuid, $2, $2, $2, c.provider_fingerprint, NULL, 'active'
     FROM ${CANONICAL_STAGE_TABLE} c
...
async function upsertEvidence(client, feedId) {
  await client.query(
    `INSERT INTO ioc_feed_source_evidence (
     ...
     SELECT c.ioc_item_id, c.observable_type, $1::uuid,
            $2, $3, c.category, c.note, NULL, c.provider_metadata, c.provider_fingerprint
     FROM ${CANONICAL_STAGE_TABLE} c
     WHERE c.ioc_item_id IS NOT NULL
     ON CONFLICT (ioc_item_id, ioc_observable_type, feed_id)
     DO UPDATE SET
  ...
  for (const observableType of STORED_IOC_TYPES) {
    ...
    await client.query(
      `WITH computed AS (
         ...
         JOIN ${CANONICAL_STAGE_TABLE} c
           ON c.ioc_item_id = m.ioc_item_id
          AND c.observable_type = m.ioc_observable_type
  ...
  await client.query(
    `WITH touched AS (
     ...
     membership_state AS (
       ...
       FROM touched t
       JOIN ioc_items i
         ON i.observable = t.observable
        AND i.observable_type = t.observable_type
       LEFT JOIN ioc_feed_memberships m
...
export async function finalizeUsomImport(client, {
  ...
  try {
    ...
    await buildStageCanonical(client);
    ...
    // Refresh canonical rows so newly inserted IOC ids are visible to membership writes.
    await buildStageCanonical(client);
