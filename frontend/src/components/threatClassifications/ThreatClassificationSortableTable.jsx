...
function MitreAttackBadges({ mappings, maxVisible = 2 }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center' }}>
      ...
      {hidden > 0 ? (
        <span style={{ fontSize: 11, color: '#94a3b8' }} title={mappings.slice(maxVisible).map((m) => m.id).join(', ')}>
          +{hidden}
        </span>
      ) : null}
    </div>
...
function SortableClassificationRow({
  ...
  return (
    <tr ref={setNodeRef} style={style} data-classification-id={item.id} data-locked={locked ? 'true' : 'false'}>
      ...
      <td className="tc-col-mitre" style={ui.td}>
        <MitreAttackBadges mappings={item.mitre_attack} />
      </td>
        ...
        <table className="ioc-table threat-classifications-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              <th className="tc-col-mitre" style={ui.th}>MITRE ATT&CK</th>
          ...
          <tbody>
            <SortableContext items={sortableIds} strategy={verticalListSortingStrategy}>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ ...ui.td, padding: 0 }}>{emptyState || 'No classifications found.'}</td></tr>
