...
} from './lib/updateManagementUi.js';
...
import {
  IOC_STATS_RECALCULATE_SUCCESS_TOAST,
  IOC_STATS_RECALCULATE_ALREADY_RUNNING_TOAST,
  iocStatsRecalculateButtonLabel,
  shouldDisableIocStatsRecalculateButton,
  runIocStatsRecalculateRequest
} from './lib/iocStatsRecalculate.js';
import {
...
} from './lib/threatActorTableDisplay.js';
import {
...
import {
  ...
  defaultTypeOverridesDraft,
  buildExpirationFullPatchPayload,
  buildExpirationTypePoliciesPayload,
...
function SessionProvider({ children }) {
  ...
  const refreshSession = useCallback(async () => {
    try {
      ...
      if (em) {
        ...
        setIsSystemAdmin(u.isSystemAdmin === true);
        setAuthState('authed');
      } else {
        setAuthState('anon');
  ...
  const value = useMemo(
    ...
    [authState, userEmail, userId, role, canWrite, isAdmin, isSystemAdmin, mustChangePassword, refreshSession]
  );
        ...
        >
          ...
          {!editing ? (
            <p style={{ ...ui.modalSub, marginTop: -6 }}>
              Choose the IOC type, filters, Window, and delivery limits. The feed URL template is shown after creation.
            </p>
          ) : null}
          ...
          <form onSubmit={saveFeed}>
            ...
            >
              {form.filter_mode === 'query' ? (
                ...
                <>
                  ...
                  <FeedFormField ui={ui} label="Window" helper="Time range of IOCs included in this published feed.">
            ...
            <FeedFormSection title="Delivery">
              ...
              <FeedFormField ui={ui} label="Refresh Interval (minutes)" helper="How often this feed is refreshed. Minimum 5 minutes.">
      ...
      {showFormModal ? (
        ...
        >
          <form id="threat-classification-form" onSubmit={submitForm} noValidate>
            ...
            {editingItem?.system_default && Array.isArray(editingItem?.mitre_attack) && editingItem.mitre_attack.length ? (
              <FeedFormField ui={ui} label="MITRE ATT&CK" helper="Curated reference mappings for built-in classifications." fullWidth>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {editingItem.mitre_attack.map((item) => (
                    <a
                      key={item.id}
                      href={item.url}
                      target="_blank"
...
function ThreatActorManagerPage() {
  ...
  const [searchParams, setSearchParams] = useSearchParams();
  const ui = PUBLISHED_FEEDS_UI;
  const initial = useMemo(() => parseThreatActorManagerUrlState(searchParams), []);
  const [actors, setActors] = useState([]);
  const [pagination, setPagination] = useState({
    page: initial.page,
    page_size: THREAT_ACTOR_MANAGER_PAGE_SIZE,
    total_items: 0,
    total_pages: 1,
    has_previous: false,
    has_next: false
  ...
  const [saving, setSaving] = useState(false);
  const [formError, setFormError] = useState('');
  const [descriptionModalActor, setDescriptionModalActor] = useState(null);
  const requestSeqRef = useRef(0);
  const abortRef = useRef(null);
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const params = buildThreatActorManagerQueryParams({ page, search, showInactive });
      const { data } = await api.get('/admin/threat-actors', { params, signal: controller.signal });
      if (seq !== requestSeqRef.current) return;
      const items = Array.isArray(data?.threat_actors) ? data.threat_actors : [];
      const pag = data?.pagination || {
        page,
        page_size: THREAT_ACTOR_MANAGER_PAGE_SIZE,
        total_items: items.length,
      ...
      setActors(items);
      setPagination(pag);
      const clamped = clampThreatActorManagerPage(page, pag.total_items, THREAT_ACTOR_MANAGER_PAGE_SIZE);
      if (clamped !== page) setPage(clamped);
    } catch (err) {
      if (err?.code === 'ERR_CANCELED' || err?.name === 'CanceledError' || err?.name === 'AbortError') return;
      if (seq !== requestSeqRef.current) return;
  ...
  useEffect(() => {
    setSearchParams(buildThreatActorManagerUrlSearchParams({ page, search, showInactive }), { replace: true });
  }, [page, search, showInactive, setSearchParams]);

  const showingLabel = formatThreatActorManagerShowingLabel({
    page: pagination.page,
    pageSize: pagination.page_size || THREAT_ACTOR_MANAGER_PAGE_SIZE,
    totalItems: pagination.total_items
  });
  ...
  return (
    <AppShell>
      <section style={ui.section} className={THREAT_ACTOR_PAGE_CLASS}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={ui.pageTitle}>Threat Actors</h1>
          </div>
        ...
        <div className="threat-actors-table-wrap">
          <table className={`ioc-table ${THREAT_ACTOR_TABLE_CLASS}`} style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th className="ta-col-name" style={ui.th}>Name</th>
                <th className="ta-col-aliases" style={ui.th}>Aliases</th>
                <th className="ta-col-description" style={ui.th}>Description</th>
                <th className="ta-col-active" style={ui.th}>Active</th>
                <th className="ta-col-created" style={ui.th}>Created At</th>
                <th className="ta-col-actions" style={ui.th}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading && !actors.length ? (
                ...
                <tr><td colSpan={6} style={{ ...ui.td, padding: 0 }}>
                  <EmptyState
                    title={search ? 'No matching threat actors' : 'No threat actors yet'}
                    description={search ? 'Try a different search term.' : 'Add a threat actor to link them to IOCs.'}
                    ctaLabel="Add Threat Actor"
                    canWrite={isAdmin && !search}
                    onCta={openCreateModal}
                  />
                </td></tr>
              ) : actors.map((actor) => {
                const descriptionCell = formatThreatActorDescriptionCell(actor.description);
                const aliasesCell = formatThreatActorAliasesCell(actor.aliases);
                return (
                <tr key={actor.id} className="threat-actors-row" style={{ opacity: actor.active ? 1 : 0.62 }}>
                  <td className="ta-col-name" style={ui.td}>
                    <div style={{ fontWeight: 600 }}>{actor.name}</div>
                    <div style={{ fontSize: 11, color: '#64748b' }}>{actor.slug || actor.name}</div>
                  </td>
                  <td className="ta-col-aliases" style={ui.td}>
                    <span
                      className={aliasesCell.isPlaceholder ? 'ta-aliases-static' : THREAT_ACTOR_ALIASES_CLAMP_CLASS}
                      title={aliasesCell.title}
                    >
                      {aliasesCell.displayText}
                    </span>
                  </td>
                  <td className="ta-col-description" style={ui.td}>
                    {descriptionCell.isPlaceholder ? (
                      <span className={`ta-description-static ${THREAT_ACTOR_DESCRIPTION_CLAMP_CLASS}`}>
                        {descriptionCell.displayText}
                      </span>
                    ) : descriptionCell.expandable ? (
                      <button
                        type="button"
                        className={`ta-description-button ${THREAT_ACTOR_DESCRIPTION_CLAMP_CLASS}`}
                        onClick={() => {
                          const modalState = buildThreatActorDescriptionModalState(actor);
                          if (modalState) setDescriptionModalActor(modalState);
                        }}
                        aria-label={`View full description for ${actor.name}`}
                      >
                        {descriptionCell.displayText}
                      </button>
                    ) : (
                  ...
                  <td className="ta-col-active" style={ui.td}>{actor.active ? 'Yes' : 'No'}</td>
                  <td className="ta-col-created" style={ui.td}>{formatUserDateTime(actor.created_at)}</td>
                  <td className="ta-col-actions" style={ui.td}>
                    <div className="ta-action-buttons">
                      <button type="button" style={ui.btnCompact} onClick={() => openEditModal(actor)}>Edit</button>
                      {actor.active ? (
                        <button type="button" style={ui.btnCompact} onClick={() => disableActor(actor).catch(() => {})}>Disable</button>
                      ) : (
                        <button type="button" style={ui.btnCompact} onClick={() => enableActor(actor).catch(() => {})}>Enable</button>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      {descriptionModalActor ? (
        <ModalOverlay
          size="md"
          title={threatActorDescriptionModalTitle(descriptionModalActor.name)}
          onClose={() => setDescriptionModalActor(null)}
          footer={(
            <button
              type="button"
              className={buttonClassName({ variant: 'secondary' })}
              data-modal-cancel
              onClick={() => setDescriptionModalActor(null)}
            >
              Close
            </button>
          )}
        >
          <div style={{ whiteSpace: 'pre-wrap', lineHeight: 1.55, color: '#e2e8f0' }}>
            {descriptionModalActor.description}
          </div>
        </ModalOverlay>
      ) : null}
    </AppShell>
...
function AdministrationSettingsPage() {
  const { role, userId, refreshSession, isAdmin, isSystemAdmin, authState } = useSession();
  const requestConfirm = useAppConfirm();
  const feedback = useAppFeedback();
  const ui = PUBLISHED_FEEDS_UI;
  const updatesUi = settingsLifecycleVisibility({ authState, isSystemAdmin, isAdmin });
  const canManageUpdates = canManageProductUpdates({ authState, isSystemAdmin });
  const showUpdatesSection = updatesUi.showUpdatesSection;
  ...
  const updateNotice = canManageUpdates ? formatUpdateAvailabilityNotice(updateStatus) : null;
  ...
  return (
    <AppShell>
      <section style={ui.section}>
        ...
        {updateNotice ? (
          <p style={{ margin: '12px 0 0', fontSize: 13, color: '#cbd5e1' }}>
            {updateNotice} Open Updates below for details. Upgrades are performed on the host with the installer CLI.
          </p>
        ) : null}
        ...
        {showUpdatesSection ? (
          <div style={{ ...ui.formPanel, marginTop: 16 }}>
            <h2 style={{ ...ui.formTitle, marginBottom: 6 }}>Updates</h2>
            {canManageUpdates ? (
              <>
                <p style={{ margin: '0 0 16px', fontSize: 13, color: '#94a3b8', lineHeight: 1.45 }}>
                  Update checks are informational only. Host upgrades use{' '}
                  <code style={{ fontSize: 12 }}>sudo ./installation.sh --upgrade</code>.
                </p>
                <div style={{ fontSize: 14, marginBottom: 12, lineHeight: 1.55 }}>
                  <div>
                    <strong>Current version</strong>
                    <span style={{ marginLeft: 12, color: '#e2e8f0' }}>{updateStatus?.currentVersion || productVersion?.version || '—'}</span>
                  </div>
                  <div style={{ marginTop: 4 }}>
                    <strong>Release channel</strong>
                    <span style={{ marginLeft: 12, color: '#e2e8f0' }}>{formatReleaseChannelLabel(updateStatus?.channel || productVersion?.channel)}</span>
                  </div>
                  <div style={{ marginTop: 4 }}>
                    <strong>Latest version</strong>
                    <span style={{ marginLeft: 12, color: '#e2e8f0' }}>{updateStatus?.latestVersion || '—'}</span>
                  </div>
                  <div style={{ marginTop: 4 }}>
                    <strong>Last checked</strong>
                    <span style={{ marginLeft: 12, color: '#e2e8f0' }}>
                      {updateStatus?.lastCheckedAt ? formatUserDateTime(updateStatus.lastCheckedAt) : '—'}
                    </span>
                  </div>
                  {updateStatus?.status === 'unknown' && updateStatus?.lastSuccessfulCheckAt ? (
                    <div style={{ marginTop: 4 }}>
                      <strong>Last successful check</strong>
                      <span style={{ marginLeft: 12, color: '#e2e8f0' }}>
                        {formatUserDateTime(updateStatus.lastSuccessfulCheckAt)}
                      </span>
                    </div>
                  ) : null}
                  <div style={{ marginTop: 10, color: '#cbd5e1' }}>
                    {formatUpdateStatusMessage(updateStatus)}
                  </div>
                  {updateStatus?.automaticChecksEnabled === false ? (
                    <div style={{ marginTop: 6, fontSize: 13, color: '#94a3b8' }}>
                      Automatic update checking is disabled in configuration.
                    </div>
                  ) : null}
                  {updateError ? (
                    <div style={{ marginTop: 8, color: '#fca5a5', fontSize: 13 }}>{updateError}</div>
                  ) : null}
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  {updateStatus?.releaseUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      style={{ ...ui.btn, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}
                    >
                      View Release Notes
                    </a>
                  ) : null}
              ...
              <>
                <p style={{ margin: '0 0 12px', fontSize: 13, color: '#94a3b8', lineHeight: 1.45 }}>
                  Update checks are informational only.
                </p>
                <p style={{ margin: '0 0 16px', fontSize: 13, color: '#cbd5e1', lineHeight: 1.45 }}>
                  {UPDATE_MANAGEMENT_RESTRICTED_MESSAGE}
                </p>
                <span title={UPDATE_MANAGEMENT_DISABLED_HINT} style={{ display: 'inline-flex' }}>
                  <button
                    type="button"
                    style={ui.btn}
                    disabled
                    aria-disabled="true"
                    title={UPDATE_MANAGEMENT_DISABLED_HINT}
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                    }}
...
function IOCListPage() {
  ...
  const statsRecalcSubmitRef = useRef(false);
  const statsRefreshBusyRef = useRef(false);
  const statsMetaInProgressRef = useRef(false);
  ...
  const loadSummary = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const inProgress = Boolean(data?.refresh_in_progress);
      statsMetaInProgressRef.current = inProgress;
      setStatsMeta({
      ...
      statsMetaInProgressRef.current = false;
  ...
  function setStatsBusy(next) {
    statsRefreshBusyRef.current = next;
    setStatsRefreshBusy(next);
  }

  function startStatsPoll({ notifySuccess = false } = {}) {
    stopStatsPoll();
    let attempts = 0;
    statsPollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (!data?.refresh_in_progress) {
          stopStatsPoll();
          setStatsBusy(false);
          if (notifySuccess && data?.calculated_at && !data?.missing) {
            feedback.success(IOC_STATS_RECALCULATE_SUCCESS_TOAST);
          }
        }
      } catch {
  ...
  async function requestRecalculateStats() {
    if (statsRecalcSubmitRef.current) return;
    statsRecalcSubmitRef.current = true;
    try {
      const result = await runIocStatsRecalculateRequest({
        canWrite,
        isBlocked: () => statsRefreshBusyRef.current || statsMetaInProgressRef.current,
        requestConfirm,
        postRefresh: () => api.post('/ioc/stats/refresh'),
        setBusy: setStatsBusy,
        onOutcome: (kind, payload) => {
          if (kind === 'started') {
            statsMetaInProgressRef.current = true;
            setStatsMeta((prev) => ({ ...prev, refresh_in_progress: true }));
            loadSummary({ silent: true }).catch(() => {});
            startStatsPoll({ notifySuccess: true });
          } else if (kind === 'already_running') {
            feedback.info(IOC_STATS_RECALCULATE_ALREADY_RUNNING_TOAST);
            statsMetaInProgressRef.current = true;
            setStatsMeta((prev) => ({ ...prev, refresh_in_progress: true }));
            loadSummary({ silent: true }).catch(() => {});
            startStatsPoll({ notifySuccess: true });
          } else if (kind === 'error') {
            // Modal keeps the error via AppConfirm; also surface the standard toast.
            feedback.error(apiErrorMessage(payload, 'Failed to start IOC statistics recalculation'));
          }
        }
      });
      if (result.reason === 'cancelled' || result.reason === 'blocked') {
        return;
      }
    } finally {
      statsRecalcSubmitRef.current = false;
    }
  }
  ...
  useEffect(() => {
    loadSummary()
      .then((data) => {
        if (data?.refresh_in_progress) {
          setStatsBusy(true);
          startStatsPoll({ notifySuccess: false });
        }
      })
      .catch(() => {});
  ...
  const statsRecalcDisabled = shouldDisableIocStatsRecalculateButton({
    canWrite,
    busy: statsRefreshBusy,
    refreshInProgress: statsMeta.refresh_in_progress
  });
      ...
      <div style={{ marginBottom: 14, padding: '12px 14px', border: '1px solid #334155', borderRadius: 10, background: '#0f172a' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          ...
          {canWrite ? (
            <button
              type="button"
              disabled={statsRecalcDisabled}
              onClick={() => requestRecalculateStats().catch(() => {})}
              style={{
                padding: '6px 10px',
                borderRadius: 8,
                border: '1px solid #475569',
                ...
                fontSize: 12,
                fontWeight: 600,
                cursor: statsRecalcDisabled ? 'not-allowed' : 'pointer',
                opacity: statsRecalcDisabled ? 0.72 : 1
              }}
            >
              {iocStatsRecalculateButtonLabel({
                busy: statsRefreshBusy,
    ...
    <>
      <style>{`
        ...
        * { scrollbar-color: #334155 #0b1220; }
        ...
        .threat-classifications-table .tc-col-mitre { width: 120px; min-width: 100px; }
        ...
        .threat-actors-page .threat-actors-table-wrap {
          margin-top: 16px;
          overflow-x: auto;
        }
        .threat-actors-page .threat-actors-table {
          width: 100%;
          table-layout: fixed;
          font-size: 13px;
        }
        .threat-actors-page .threat-actors-table .ta-col-name { width: 16%; min-width: 140px; }
        .threat-actors-page .threat-actors-table .ta-col-aliases { width: 20%; min-width: 160px; }
        .threat-actors-page .threat-actors-table .ta-col-description { width: auto; }
        .threat-actors-page .threat-actors-table .ta-col-active { width: 72px; }
        .threat-actors-page .threat-actors-table .ta-col-created { width: 160px; }
        .threat-actors-page .threat-actors-table .ta-col-actions { width: 150px; }
        .threat-actors-page .threat-actors-table tbody tr {
          border-bottom: 1px solid #334155;
        }
        .threat-actors-page .threat-actors-table tbody tr:hover {
          background: rgba(30, 41, 59, 0.55);
        }
        .threat-actors-page .threat-actors-table td {
          vertical-align: middle;
        }
        .threat-actors-page .ta-clamp-description,
        .threat-actors-page .ta-clamp-aliases {
          display: -webkit-box;
        ...
        .threat-actors-page .ta-description-button {
          display: block;
          width: 100%;
          padding: 0;
          margin: 0;
        ...
        .threat-actors-page .ta-description-button:hover {
          color: #cbd5e1;
        }
        .threat-actors-page .ta-description-button:focus-visible {
          outline: 2px solid #93c5fd;
          outline-offset: 2px;
          border-radius: 4px;
        }
        .threat-actors-page .ta-description-static,
        .threat-actors-page .ta-aliases-static {
          display: block;
        }
        .threat-actors-page .ta-col-actions {
          white-space: nowrap;
          vertical-align: middle;
        }
        .threat-actors-page .ta-action-buttons {
          display: inline-flex;
          flex-wrap: nowrap;
          gap: 6px;
          align-items: center;
        }
        .threat-actors-page .ta-action-buttons button {
          white-space: nowrap;
        }
        @media (max-width: 980px) {
          .threat-actors-page .threat-actors-table .ta-col-created { display: none; }
          .threat-actors-page .threat-actors-table { min-width: 760px; }
        }
        .app-feedback-stack {
          position: fixed;
          top: var(--th-shell-pad);
          left: 0;
          right: 0;
          /* Below modal overlays (1000+) and confirms (1200); above page chrome. */
          z-index: 900;
          display: flex;
          flex-direction: column;
          align-items: center;
        ...
        @media (min-width: 1024px) {
          /* Center over main content, not the full viewport (sidebar is in-flow). */
          body:has(.app-shell > .sidebar) .app-feedback-stack {
            padding-left: calc(var(--th-shell-pad) + var(--th-sidebar-width) + var(--th-shell-gap));
            padding-right: var(--th-shell-pad);
          }
        }
        .app-feedback {
          pointer-events: auto;
        ...
        @media (max-width: 720px) {
          ...
          .threat-classifications-table .tc-col-mitre { display: none; }
        ...
        .mobile-topbar {
          ...
          gap: 12px;
          padding: 0 14px;
          height: var(--th-mobile-topbar-height);
          border-bottom: 1px solid #334155;
          position: sticky;
          top: 0;
          z-index: 100;
        ...
        @media (max-width: 1023px) {
          ...
          .app-feedback-stack {
            top: calc(var(--th-mobile-topbar-height) + 8px);
          }
        }
